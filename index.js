...
let led;
let ledEncendido = false; // Variable booleana para llevar un seguimiento del estado del LED

// Cuando la placa Arduino esté lista, se ejecutará este código
board.on("ready", () => {
    console.log("¡Arduino Conectado!");

    // Se define el pin 8 como un LED
    led = new five.Led(8);
    ...
    // Cuando el botón de encender se presione, se enciende el LED si no está encendido
    buttonOn.on("press", () => {
        if (!ledEncendido) {
            led.on();
            console.log("LED encendido");
            ledEncendido = true; // Se actualiza el estado del LED
        }
    });

    // Cuando el botón de apagar se presione, se apaga el LED y se resetea el estado
    buttonOff.on("press", () => {
        led.off();
        console.log("LED apagado");
        ledEncendido = false; // Se resetea el estado del LED
    });
});
...
app.get("/", async(req, res) => {
    ...
    const resultado = await fetch(apiUrl + "/api/v1/personas");
